...
namespace UserManagement.Model
{
    ...
    {
        public SalaryBetweenDate GetSalaryWithCalculatedElementsAtDate(DateTime currentDate, SalaryRising[] allSalaryRisings, CalculatedElementUser[] allCalculatedElementUsers, Indemnity[] allIndemnities)
        {
            ...
            // Filter by year
            for (int i = filteredElements.Count - 1; i >= 0; i--)
            {
                inList = filteredElements[i].YearsSplittedBySemiColon.Contains(currentDate.Year.ToString());

                for (int j = 0; j < filteredElements[i].YearIntervals.Count; j++)
                {
                    if (currentDate.Year >= filteredElements[i].YearIntervals[j][0] && currentDate.Year <= filteredElements[i].YearIntervals[j][1])
                    {
                        inIntervals = true;
                        break;
                    }
            ...
            for (int i = filteredElements.Count - 1; i >= 0; i--)
            {
                inList = filteredElements[i].MonthsSplittedBySemiColon.Contains(currentDate.Month.ToString());

                for (int j = 0; j < filteredElements[i].MonthIntervals.Count; j++)
                {
                    if (currentDate.Month >= filteredElements[i].MonthIntervals[j][0] && currentDate.Month <= filteredElements[i].MonthIntervals[j][1])
                    {
                        inIntervals = true;
                        break;
                    }
            ...
            // Filter by user
            filteredElements = filteredElements.Where(element => element.UserList.Contains(Id.ToString())).ToList();

            filteredElements = filteredElements.GroupBy(
                element => element.Indemnity
                ).Select(
                    grouped => grouped.OrderByDescending(
                        row => row.BeginningDate).First()
                    ).ToList();

            // Assign value to each type of Indemnity
            List<CalculatedElementUser> uniqueCalculatedElementsByIndeminity = new List<CalculatedElementUser>();    
            foreach (Indemnity indemnity in allIndemnities)
            {
                uniqueCalculatedElementsByIndeminity.Add(new CalculatedElementUser(indemnity.Name));
            }

            foreach (CalculatedElementUser filteredElement in filteredElements)
            {
                for (int i = 0; i < uniqueCalculatedElementsByIndeminity.Count; i++)
                {
                    if (uniqueCalculatedElementsByIndeminity[i].Indemnity.Equals(filteredElement.Indemnity))
                        uniqueCalculatedElementsByIndeminity[i] = filteredElement;
                }
            }

        /// Treat formula for each calculated elements
            Dictionary<string, decimal> indemnityValues = new Dictionary<string, decimal>();
            
            Expression expression = null;
            foreach (CalculatedElementUser element in uniqueCalculatedElementsByIndeminity)
            {
                if (string.IsNullOrEmpty(element.Formula))
                    continue;

                expression = new Expression(element.Formula);
                expression.Parameters["salaire"] = nearest.Salary;
            }

            result = new SalaryBetweenDate(currentDate, nearest.Salary, null)
            {
                elementsApplicated = uniqueCalculatedElementsByIndeminity
            };
